			...
			if (!isExtended) {
				...
				var getDay = function (year, month) {
					return Months[month] + 365 * (year - 1970) + floor((year - 1969 + (
							month = +(month > 1))) / 4) - floor((year - 1901 + month) / 100) +
			...
			forEach = function (object, callback) {
				...
				if (!size) {
					...
					forEach = function (object, callback) {
						...
						for (property in object) {
							...
							if (!(isFunction && property == "prototype") && !isProperty.call(
								members, property) && (members[property] = 1) && isProperty.call(
								object, property)) {
								callback(property);
							}
						}
					...
					forEach = function (object, callback) {
						...
						for (property in object) {
							if (!(isFunction && property == "prototype") && isProperty.call(
								object, property) && !(isConstructor = property === "constructor")) {
								callback(property);
							}
						}
							...
							if (value > -1 / 0 && value < 1 / 0) {
								...
								if (getDay) {
									...
									for (year = floor(date / 365.2425) + 1970 - 1; getDay(year + 1, 0) <=
										date; year++);
									for (month = floor((date - getDay(year, 0)) / 30.42); getDay(year,
										month + 1) <= date; month++);
								...
								value = (year <= 0 || year >= 1e4 ? (year < 0 ? "-" : "+") +
										toPaddedString(6, year < 0 ? -year : year) : toPaddedString(4,
											year)) +
					...
					if (objectTypes[typeof filter] && filter) {
						if ((className = getClass.call(filter)) == functionClass) {
							...
							for (var index = 0, length = filter.length, value; index < length; value =
								filter[index++], ((className = getClass.call(value)), className ==
						...
						default:
							...
							if (charCode >= 48 && charCode <= 57) {
								...
								if (charCode == 101 || charCode == 69) {
									...
									for (position = Index; position < length && ((charCode = source.charCodeAt(
										position)), charCode >= 48 && charCode <= 57); position++);
									if (position == Index) {
